...
新增 **GB28181 国标接入（设备注册/目录/INVITE 拉流，RTP/PS 解析）、Home Assistant（MQTT 自动发现）集成、
以及类传统硬件 NVR 的「电视墙」多分屏监看页面** —— 全部仍为纯 Go、无 CGO。
...
| **GB28181 国标接入** | 内置 **GB/T 28181 SIP 平台**：摄像头/NVR **注册**上来，自动**查询目录**列出通道；选定通道后通过 **INVITE** 拉流（RTP/UDP 承载 **MPEG-PS**，纯 Go 解析出 H.264/H.265），接入后与其它来源一样录像/监看/再分发。SIP 摘要鉴权、Keepalive、目录、INVITE/ACK/BYE 均为**手写纯 Go 子集**（见下「已知范围」）。 |
| **Home Assistant 集成** | 通过 **MQTT 自动发现（discovery）** 把每路摄像头注册为 HA 设备：**移动 binary_sensor** + **在线/连通性 binary_sensor**，状态实时推送。复用「通知告警」里的 MQTT broker，纯 MQTT 发布、零额外依赖。 |
| **电视墙** | 类传统硬件 NVR 的多分屏监看页：**1 / 4 / 9 / 16 分屏**，把任意摄像头拖入格子、整墙全屏、布局自动保存（服务端持久化）。每格复用低延迟 MSE 直播。 |
...
- GB28181（SIP 平台 + RTP 收流 + MPEG-PS 解复用）：**手写纯 Go 子集**，用 `github.com/pion/rtp` 解 RTP、`golang.org/x/text` 解码 GB2312/GBK 目录名，复用 `mediacommon` 的 H.264/H.265 工具切分访问单元。
...
### 接入 GB28181 国标设备
1. 在 `config.yaml` 开启 `gb28181.enabled: true`，设置 `server_id`（20 位平台编号）、`domain`、`password`（设备注册口令）、`media_ip`（设备能访问到的本机 IP，留空自动探测）并重启。
2. 在摄像头/NVR 的「国标/GB28181」参数里填写：SIP 服务器编号＝`server_id`、SIP 域＝`domain`、SIP 服务器地址/端口＝`media_ip:5060`、注册口令＝`password`。系统设置页的「GB28181 国标接入」卡片会原样显示这些值。
3. 设备注册成功后，摄像头管理 → 添加 → 来源类型选 **GB28181 国标接入**，从下拉中选择已注册设备与通道（单通道设备可留空）。保存后即按 INVITE 拉流、与其它来源一致地录像/监看/再分发。

> 媒体为 RTP/UDP 承载的 MPEG-PS，系统在纯 Go 中解复用出 H.264/H.265。当前为首个可用版本，**仅视频（不含 GB28181 音频）、媒体走 UDP**，详见末尾「已知范围」。

### Home Assistant 集成（MQTT 自动发现）
先在「通知告警」里配置好 MQTT Broker（与 Home Assistant 同一个），再到系统设置 → 「Home Assistant 集成」启用。系统会为每路摄像头发布 HA discovery 配置：一个**移动 binary_sensor**与一个**在线/连通性 binary_sensor**，并实时推送移动/在线状态。HA 端开启 MQTT 集成后会自动出现这些设备与实体，可直接用于自动化。

### 电视墙
左侧导航「电视墙」：选择 **1 / 4 / 9 / 16 分屏**，点击空格挑选摄像头、悬停画面可切换或移除、右上「全屏」铺满屏幕。布局会自动保存到服务端，下次进入恢复。适合值班大屏集中监看。
...
GET    /api/gb28181/info                GB28181 平台信息（设备端需配置的 server_id/domain/地址）
GET    /api/gb28181/devices             已注册的国标设备及其通道目录
POST   /api/gb28181/devices/{id}/refresh 重新查询设备通道目录 (operator)
...
GET/PUT /api/settings/{retention|s3|recording|notifications|homeassistant}  (admin)
POST   /api/settings/s3/test            测试 S3 连接
POST   /api/settings/notifications/test 发送测试通知
GET    /api/settings/videowall          电视墙布局（viewer 可读）
PUT    /api/settings/videowall          保存电视墙布局 (operator)
GET    /api/me                          当前用户与角色
...
internal/gb28181   GB28181 国标平台：SIP（注册/鉴权/目录/INVITE）+ RTP 收流 + MPEG-PS 解复用
internal/hadiscovery Home Assistant MQTT 自动发现（摄像头设备 + 移动/在线 binary_sensor）
...
告警通知、多协议再分发、WebRTC、双向对讲，以及 **GB28181 国标接入、Home Assistant 集成、电视墙**。
以下为已知边界（便于后续迭代）：
...
9. **GB28181 为首个可用版本**：实现了设备注册（摘要鉴权）、Keepalive、目录查询、INVITE/ACK/BYE 与 RTP/UDP+MPEG-PS 收流，**仅视频（H.264/H.265），暂不含 GB28181 音频、媒体 TCP 承载、历史回放与级联**；SIP 为手写纯 Go 子集（已对消息解析/摘要/SDP/PS 解复用做单元测试），真实设备的端到端联调仍待在实际国标设备上验证。Docker 下 GB28181 建议 host 网络。
10. **Home Assistant 集成走 MQTT 自动发现**：发布移动/在线两个 binary_sensor，复用通知用的 MQTT broker；暂未发布快照/视频实体（HA 端可直接用 RTSP 再分发地址作为摄像头源）。
11. **AI 目标检测 / 人脸识别 / 车牌识别**本版本仅产出**实现方案设计文档**（见 [`docs/ai-detection-design.md`](docs/ai-detection-design.md)）：结论是在保持 CGO-free 的前提下，以「外部检测 sidecar（HTTP/gRPC）+ kenko 负责抽帧/运动门控/事件集成」落地，尚未编码实现。
