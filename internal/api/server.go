...
func (s *Server) router() http.Handler {
	...
	r.Route("/api", func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			...
			r.Get("/face/status", s.handleFaceStatus)
			...
			r.Group(func(r chi.Router) {
				...
				r.Post("/face/scan", s.handleFaceScan)
				r.Post("/face/recluster", s.handleRecluster)
				r.Patch("/persons/{id}", s.handlePatchPerson)
				r.Delete("/persons/{id}", s.handleDeletePerson)
				r.Post("/persons/merge", s.handleMergePersons)
				r.Post("/persons/{id}/split", s.handleSplitPerson)
				r.Post("/tracks/{id}/assign", s.handleAssignTrack)
				r.Post("/tracks/{id}/confirm", s.handleConfirmTrack)
				r.Post("/faces/{id}/ignore", s.handleIgnoreFace)
				r.Post("/face/links", s.handleCreateLink)
				r.Delete("/face/links/{id}", s.handleDeleteLink)
			...
			r.Group(func(r chi.Router) {
				...
				r.Get("/settings/face", s.handleGetFaceSettings)
				r.Put("/settings/face", s.handleSetFaceSettings)
